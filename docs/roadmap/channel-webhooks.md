...
Env: `TWILIO_AUTH_TOKEN`, `TWILIO_NUMBER_MAP`, `RUNNRR_TWILIO_SKIP_VERIFY` (non-prod only). Add all three to `.env.example` with empty values.
...
- `test_skip_verify_only_outside_production` — RUNNRR_TWILIO_SKIP_VERIFY=1 bypasses the check when ENV != production; in production the flag is ignored and a missing token fails startup.
