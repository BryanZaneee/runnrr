...
- [`backend/app.py`](backend/app.py) — FastAPI: POST `/api/chat` (SSE), GET `/api/models`, GET `/api/health`, GET `/api/budget`, GET `/api/profile`, GET `/api/profiles` (lists all bundled profiles for the agent switcher; logs and skips unloadable profiles), GET `/api/rag/index`, GET `/api/status`. **`/api/profile` is intentionally cheap** — it does NOT embed RAG-index health (that scans/hashes the KB). RAG health is the canonical job of `GET /api/rag/index`.
...
- `profiles/` — reusable agent profiles loaded by the engine. `personal-agent` is the bundled personal showcase (default); `customer-service` ships its own self-contained KB; `sales-concierge` ships a `data/catalog.json`; `research-analyst` and `sales-concierge` also ship per-profile `evals/smoke.json`; `frampton` uses the tracked Fextralife scrape at `kb/frampton`.
