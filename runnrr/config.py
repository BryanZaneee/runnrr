"""Runnrr config — env loading + model registry.
...
from runnrr.types import AvailableModel, ModelConfig

load_dotenv()

KB_ROOT: Path = Path(os.environ.get("RUNNRR_KB_ROOT", "./kb")).resolve()
PROFILE_ROOT: Path = Path(os.environ.get("RUNNRR_PROFILE_ROOT", "./profiles")).resolve()
...
TOOL_DEBUG_ERRORS: bool = os.environ.get("RUNNRR_TOOL_DEBUG_ERRORS", "0") == "1"

EMBEDDING_BACKEND: str = os.environ.get(
    "RUNNRR_EMBEDDING_BACKEND", "voyage"
).strip().lower()
EMBEDDING_MODEL: str = os.environ.get("RUNNRR_EMBEDDING_MODEL", "").strip()
...
EMBED_BATCH_SIZE: int = max(1, int(os.environ.get("RUNNRR_EMBED_BATCH_SIZE", "96")))
_rag_index_root = os.environ.get("RUNNRR_RAG_INDEX_ROOT", "").strip()
RAG_INDEX_ROOT: Path | None = (
    Path(_rag_index_root).expanduser().resolve() if _rag_index_root else None
)

GRADER_MODEL_ID: str = os.environ.get("RUNNRR_GRADER_MODEL", "claude-haiku-4-5").strip()
...
RERANK_ENABLED: bool = os.environ.get("RUNNRR_RERANK", "0") == "1"
...
# unhonorable one RAISES. Prices deliberately live in runnrr/pricing.py, where
